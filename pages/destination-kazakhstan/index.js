...
import heroDestinationData from "../../public/json/data/hero_destination_kazakhstan.json";
import destinationDetailData from "../../public/json/data/destination_kazakhstan_product.json";
  ...
  return {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: "Kazakhstan",
        item: `${siteUrl}/destination-kazakhstan`,
      },
    ],
  };
}
...
const DESTINATION_NAME = "Kazakhstan";
...
function formatRecommendedTour(tour) {
  return {
    ...
    short_des:
      ...
      "Discover Kazakhstan with Belet Travel.",
...
async function fetchKazakhstanTours(client) {
...
export default function Destination({
  ...
  useEffect(() => {
    ...
    const loadRecommendedTours = async () => {
      try {
        const tours =
          await fetchKazakhstanTours(supabase);
        ...
        console.error(
          "Client Kazakhstan recommended tours error:",
          error
        );
      } finally {
        if (isMounted) {
  ...
  return (
    <>
      <Head_Meta
        meta_data={siteMetaData.destination_kazakhstan_meta}
...
export async function getStaticProps() {
  ...
  try {
    recommendedTours =
      await fetchKazakhstanTours(serverSupabase);
  } catch (error) {
    console.error(
      "Server Kazakhstan recommended tours error:",
      error
    );
  }
