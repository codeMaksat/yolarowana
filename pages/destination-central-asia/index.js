...
function formatRecommendedTour(tour) {
  return {
    ...
    short_des:
      ...
      "Discover Central Asia with Belet Travel.",
...
async function fetchCentralAsiaTours(client) {
...
export default function Destination({
  ...
  const { data: heroDestinationData } = useFetchData(
    "/json/data/hero_destination_central-asia.json"
  );

  const { data: destinationDetailData } = useFetchData(
    "/json/data/destination_central_asia_product.json"
  );
  ...
  useEffect(() => {
    ...
    const loadRecommendedTours = async () => {
      try {
        const tours =
          await fetchCentralAsiaTours(supabase);
        ...
        console.error(
          "Client Central Asia recommended tours error:",
          error
        );
      } finally {
        if (isMounted) {
  ...
  return (
    <>
      <Head_Meta
        meta_data={
          seoData?.destination_central_asia_meta
        }
        comman_meta={seoData}
      />
...
export async function getStaticProps() {
  ...
  try {
    recommendedTours =
      await fetchCentralAsiaTours(serverSupabase);
  } catch (error) {
    console.error(
      "Server Central Asia recommended tours error:",
      error
    );
  }
