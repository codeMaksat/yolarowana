...
async function fetchDestinationTours(client) {
...
export default function Destination({
  ...
  const [recommendedToursLoading, setRecommendedToursLoading] =
    useState(initialRecommendedTours.length === 0);
  ...
  useEffect(() => {
    ...
    const loadRecommendedTours = async () => {
      try {
        const tours = await fetchDestinationTours(supabase);
        ...
        console.error(
          "Client Uzbekistan recommended tours error:",
          error
        );
      } finally {
        if (isMounted) {
  ...
  return (
    <>
      ...
      <All_Destination_Detail
        ...
        recommendedToursLoading={recommendedToursLoading}
      />
    </>
  );
}
...
export async function getStaticProps() {
  ...
  try {
    recommendedTours =
      await fetchDestinationTours(serverSupabase);
  } catch (error) {
    console.error(
      "Server Uzbekistan recommended tours error:",
      error
    );
  }
